...
- Bump `versionName` with the same intent, for example `1.7.0-universal-carrier`.
- Add a short entry to `CHANGELOG.md` before publishing.
- Keep Windows versioning separate under `windows/XLTD.Vpn.Windows`:
  - Android stable line: `1.7.x-universal-carrier`.
  - Windows beta line: `0.3.x-beta`.
...
The helper writes `dist/windows/XLTD_Vpn-Windows-0.3.0-beta-win-x64.zip` and prints SHA256 for the GitHub pre-release.
...
5. Upload Android APKs to stable Android tags such as `v1.7.0`.
6. Upload Windows beta zips to pre-release Windows tags such as `windows-v0.3.0-beta`.
